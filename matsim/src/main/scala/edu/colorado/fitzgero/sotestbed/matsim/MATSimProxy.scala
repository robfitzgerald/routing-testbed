...
import org.matsim.api.core.v01.population.{Leg, Person, Route}
...
trait MATSimProxy extends SimulatorOps[SyncIO] with LazyLogging { self =>
  ...
  var reasonableReplanningLeadTime: TravelTimeSeconds = TravelTimeSeconds.Zero
  var maxPathAssignments: Int = 0
              ...
              self.qSim.getEventsManager.addHandler(new VehicleEntersTrafficEventHandler {
                override def handleEvent(event: VehicleEntersTrafficEvent): Unit = {
                  ...
                  if (soReplanningThisIteration) {
                    ...
                    /////////////////////////////
                    // 20191031 - happy halloween
                    //  event.linkId is origin (should == mobsimAgent.getCurrentLinkId); mobsimAgent.getDestinationLinkId is destination
                    //  but, there's no guarantee that o/d is unique for this agent (they may go from home/work multiple times..)
                    //  however, departure time is unique. more exact, departure time + o/d link ids for a tag.
                    //  but since we are in traffic, we should be able to inspect the current leg's departure time,
                    //  and, maybe it would be useful to compare it with the event.time for our sanity.
                    //  let's try that.
...
//                    if (requestClass match { case RequestClass.SO(_) => true; case _ => false }) &&
                      ...
                      logger.debug(
                        s"${SimTime(playPauseSimulationControl.getLocalTime)} agent ${event.getPersonId}: applying stored route with ${pathFromPathStore.length} edges")
...
//                    person       <- controler.getScenario.getPopulation.getPersons.asScala.get(event.getPersonId)
                      ...
                      departureTime <- agentsInSimulationNeedingReplanningHandler.getDepartureTimeForAgent(event.getPersonId)
                      plan = WithinDayAgentUtils.getModifiablePlan(mobsimAgent)
                      leg <- plan.getPlanElements.asScala
                        .find { l =>
...
//                    if (requestClass match { case RequestClass.SO(_) => true; case _ => false })
                      ...
                      logger.debug(
                        s"${SimTime(playPauseSimulationControl.getLocalTime)} agent ${event.getPersonId}: storing completed route with ${agentExperiencedRoute.length} edges")

                      // attach this path, keyed by departure time, to the complete list
                      completePathStore.get(event.getPersonId) match {
                        case None =>
                          val thisPath: Map[MATSimProxy.DepartureTime, List[Id[Link]]] =
                            Map(departureTime -> agentExperiencedRoute)
                          completePathStore.update(event.getPersonId, thisPath)
                        case Some(alreadyHasPaths) =>
                          completePathStore.update(event.getPersonId, alreadyHasPaths.updated(departureTime, agentExperiencedRoute))
                      }
                    }
                  }
                }
                  ...
                  if (soReplanningThisIteration) {
                    ...
                    if (endOfRoutingTime <= nextSimTime) {
                      ...
                      val agentsForReplanning: List[Request] =
                        agentsInSimulationNeedingReplanningHandler.getActiveAgentsEligibleForReplanning(maxPathAssignments)
                          .foldLeft(List.empty[Request]) { (mobsimAgents, agentInSimulation) =>
                            agentsInSimulation.get(agentInSimulation) match {
                              ...
                              case Some(mobsimAgent) =>
                                ...
                                val currentEdgeId     = mobsimAgent.getCurrentLinkId
                                val destinationEdgeId = EdgeId(mobsimAgent.getDestinationLinkId.toString)
                                val leg               = WithinDayAgentUtils.getModifiableCurrentLeg(mobsimAgent)
                                val route             = leg.getRoute.asInstanceOf[NetworkRoute].getLinkIds.asScala.toList
                                MATSimProxy.selectRequestOriginLink(route, currentEdgeId, self.qSim, reasonableReplanningLeadTime) match {
  ...
  override def assignRoutes(simulator: Simulator, responses: List[Response]): SyncIO[Simulator] = SyncIO {
    ...
    if (responses.isEmpty) {
      ...
      for {
        ...
        leg           = WithinDayAgentUtils.getModifiableCurrentLeg(mobsimAgent) // agent is in simulation, so this is safe
        departureTime = MATSimProxy.DepartureTime(leg.getDepartureTime.toInt)
      } {
        ...
        // TODO: hey, use this instead? RouteUtils.createNetworkRoute(updatedRouteAsJava, qSim.getNetsimNetwork.getNetwork)

        // continue to record the experienced agent route associated with this departure time
        completePathStore.get(agentId) match {
          case None =>
            completePathStore.update(agentId, Map(departureTime -> futureRoute)) // in-place
          case Some(agentAlreadyRecordedInStore) =>
            completePathStore.update(agentId, agentAlreadyRecordedInStore.updated(departureTime, futureRoute)) // in-place
        }
...
//    }
  ...
  override def getActiveRequests(simulator: Simulator): SyncIO[List[Request]] = SyncIO {
    ...
    newRouteRequests match {
      case None => List.empty
      case Some(rr) => rr.requests
    }
//    val agentLookup  = qSim.getAgents.asScala
//    val personLookup = controler.getScenario.getPopulation.getPersons.asScala
//    for {
//
//      personId                 <- agentsInSimulationNeedingReplanningHandler.getActiveAgentsEligibleForReplanning(maxPathAssignments)
...
object MATSimProxy {
  ...
  def selectRequestOriginLink(
    ...
    currentEdgeId: Id[Link],
    qSim: QSim,
    reasonableReplanningLeadTime: TravelTimeSeconds
  ): Option[EdgeId] = {

    previousPathFromCurrentOrigin.dropWhile{_ != currentEdgeId} match {
      case Nil =>
        // stepped through agent's path but didn't find current edge; start a route plan
      ...
      case previousPath =>
        ...
        // then don't touch it.
        val reasonableStartPointFoldAccumulator: ReasonableStartPointFoldAccumulator =
          previousPath
            .map { l =>
        ...
        reasonableStartPointFoldAccumulator.startPoint
    }
  }
...
//
//  /**
//    * used for the path store tables
//    * @param origin leg origin link
//    * @param destination leg destination link
//    * @return a label for the hash map lookup
//    */
//  def createLegLabel(origin: Id[Link], destination: Id[Link]): LegLabel = s"$origin->$destination"
}
//////// GRAVEYARD
