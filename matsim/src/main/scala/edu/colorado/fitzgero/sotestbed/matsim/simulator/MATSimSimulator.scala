                  ...
                  Try {
                    ...
                    if (self.useExternalRoutingEngineForSelfishAgents &&
                      ...
                      for {
                        ...
                        homeMorningActivity <- WithinDayAgentUtils.getModifiablePlan(mobsimAgent).getPlanElements.asScala.toList.headOption.map {
                      ...
                      logger.debug(s"[VehicleEntersTrafficEventHandler] agent ${event.getPersonId} marked for routing")
                  ...
                  Try {
                    ...
                    if (self.markForUEPathOverwrite.isDefinedAt(event.getVehicleId)) {
                      ...
                      for {
                        ...
                        originLinkId      <- fullRoute.dropWhile(_ != currentLinkId).headOption // confirms MATSim currentLinkId is correct
                        ...
                        MATSimRouteOps.selectRequestOriginLink(fullRoute,
                                                               ...
                                                               self.minimumRemainingRouteTimeForReplanning) match {
                          case None =>
                            val remainingTT: Double = MATSimRouteOps.estRemainingTravelTimeSeconds(fullRoute, currentLinkId, qSim)

                            logger.debug(
                              f"[LinkEnterEventHandler] didn't find a reasonable edge to attempt replanning for agent $personId with est. remaining travel time $remainingTT%.2f seconds")
                          ...
                          case Some(reasonableStartEdgeId) =>
                            ...
                            val experiencedEdgeData: List[RouteRequestData.EdgeData]    = MATSimRouteOps.convertRouteToEdgeData(experiencedRoute, qSim)
                            val edgeDataRemainingRoute: List[RouteRequestData.EdgeData] = MATSimRouteOps.convertRouteToEdgeData(remaingingRoute, qSim)
...
//                      departureTime <- DepartureTime.getLegDepartureTime(leg)
                      ...
                      val departureTime: DepartureTime = self.departureTimeStore.getOrElse(agentId, DepartureTime(Int.MaxValue))
                      ...
                      val requestClass: RequestClass =
                        if (self.soAgentReplanningHandler.isUnderControl(agentId)) RequestClass.SO() else RequestClass.UE
                      ...
                      if (//!self.onlySelfishAgents &&
                        ...
                        logger.info(
                          s"[VehicleLeavesTrafficEventHandler] agent $agentId replanned $replannings times"
                        )
                              ...
                              (mobsimAgents, agentData) =>
                                {
                                  ...
                                  agentsInSimulation.get(personId) match {
                                    case None =>
                                      logger.debug(
                                        s"[MobsimBeforeSimStepListener] agent $agentData that emitted a departure event was not found in QSim - possibly already at destination")
                                      mobsimAgents
                                    case Some(mobsimAgent) =>
                                      Option(WithinDayAgentUtils.getModifiableCurrentLeg(mobsimAgent)) match {
                                        case None =>
                                          logger.error(
                                            s"[MobsimBeforeSimStepListener] agent $agentData that emitted a departure event does not yet have a trip Leg")
                                          mobsimAgents
                                        case Some(leg) =>
                                          ...
                                          MATSimRouteOps.selectRequestOriginLink(fullRoute,
                                            ...
                                            case None =>
                                              ...
                                              logger.debug(
                                                f"[MobsimBeforeSimStepListener] didn't find a reasonable edge to attempt replanning for agent $personId with est. remaining travel time $remainingTT%.2f seconds")
                                              soAgentReplanningHandler.incrementNumberFailedRoutingAttempts(personId)
                                              mobsimAgents
                                            case Some(sourceEdgeId) => {
                                              for {
                                                departureTime <- soAgentReplanningHandler.getDepartureTimeForAgent(personId).map { t =>
                                                  SimTime(t.value)
                                                }
                                              } yield {
                                                val thisRequest: Request =
                                                ...
                                                val experiencedEdgeData: List[RouteRequestData.EdgeData] =
                                                  MATSimRouteOps.convertExperiencedRouteToEdgeData(experiencedRoute, qSim)
                                                val remainingLinkIds: List[Id[Link]] = fullRoute.dropWhile(_ != currentLinkId)
                                                ...
                                                val remainingDistance: Meters =
                                                  MATSimRouteOps.distanceOfPath(remainingLinkIds, qSim).getOrElse(Meters.Zero)
                                                ...
                                                logger.debug(
                                                  s"[MobsimBeforeSimStepListener] requesting route for agent $personId, o=$sourceEdgeId, d=$destinationEdgeId")
            ...
            Try { updatedRoute(mobsimAgentCurrentRouteLinkIdIndex) }.toEither match {
              ...
              case Right(updatedRouteLinkAtCurrentLinkIndex) =>
                if (updatedRouteLinkAtCurrentLinkIndex != currentLinkId) {
                  logger.warn(
                    s"[assignRoutes] composed a replanning route update but found the currentLinkIndex does not correspond with the currentlinkId")
        ...
        def _isDone(): Either[IsDoneFailure, SimulatorState] = {
          if (!t.isAlive) {
            matsimState match {
              ...
              case other =>
                Left(IsDoneFailure.FinishingError(
  ...
  override def getAgentsNewlyAvailableForReplanning(simulator: Simulator): SyncIO[List[AgentBatchData]] = SyncIO {
    if (matsimState != SimulatorState.Running) {
      ...
      self.newSORouteRequests match {
        ...
        case Some(rr) =>
          logger.debug(
            s"[getActiveRequests] sending ${rr.requests.size} SO, ${ueRequests.size} UE requests, ${agentsLeftSimulationRequests.size} agents left simulation requests")
