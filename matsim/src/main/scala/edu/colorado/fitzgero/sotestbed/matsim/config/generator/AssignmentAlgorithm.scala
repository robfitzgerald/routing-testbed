    ...
    def toHocon: String = algorithm match {
      ...
      case Karma(driverPolicyMappingUnit, networkPolicyScale, bankMaxKarma) =>
        s"""algorithm = {
           ...
           |    driver-policy.type = delay-proportional
           |    network-policy.type = congestion-proportional-threshold
           |    auction-policy.type = winners-pay-all
           |    congestion-observation.type = mean-from-batch
           |    free-flow-cost-function.type = capacity-based
