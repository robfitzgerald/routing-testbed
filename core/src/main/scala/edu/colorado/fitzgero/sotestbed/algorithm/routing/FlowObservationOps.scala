...
object FlowObservationOps {
  ...
  def defaultMarginalFlow[F[_]: Monad, V, E](roadNetwork: RoadNetwork[F, V, E], path: Path): F[List[(EdgeId, Flow)]] =
    Monad[F].pure {
      path.map { pathSegment => (pathSegment.edgeId, Flow(1.0)) }
    }
  ...
  /**
    ...
    * @return
    */
  def marginalDecayedFlows[F[_]: Monad, V, E](
    decayAfterEstimatedTime: SimTime,
    decayRate: Flow
  )(roadNetwork: RoadNetwork[F, V, E], path: Path): F[List[(EdgeId, Flow)]] = {
    for {
      linkTuples <- path.traverse { pathSegment =>
        roadNetwork.edge(pathSegment.edgeId).map { edge => (pathSegment, edge) }
      }
    } yield {
      for {
