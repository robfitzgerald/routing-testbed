      ...
      val experiment: F[ExperimentState] = startState.iterateUntilM {
        case ExperimentState(e0, r0, b0, s0, _) =>
          for {
            ...
            (ueRequests, soUpdate) = batchDataUpdate.partition { payload => selfishOnly || BatchingManager.splitUEFromSO(payload) }
...
object RoutingExperiment {
  def runUE[F[_]: Monad, V, E](
    ...
    val ueRouteRequests: List[Request] = ueRequests.flatMap{
      case data: AgentBatchData.RouteRequestData => data.request.requestClass match {
